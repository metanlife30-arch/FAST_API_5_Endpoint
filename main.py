...
from schemas import WalletСhange, WalletResponce
...
async def wallet_add()->WalletResponce:
...
async def wallet_change(wallet:Annotated[WalletСhange,Query()])->WalletResponce:
    async with async_session_local() as session:
...
async def wallet_get(id: Annotated[int,Path(..., title="id wallet", description="Get a wallet by ID")])->WalletResponce:
    async with async_session_local() as session:
        # Поиск кошелька по ID
